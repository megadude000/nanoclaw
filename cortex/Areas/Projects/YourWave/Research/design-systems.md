...
**Container Widths**
```css
--container-prose: 65ch;    /* For long-form text (blog, product descriptions) */
--container-content: 960px; /* Standard content width */
--container-wide: 1200px;   /* Dashboard and catalog views */
--container-full: 1400px;   /* Maximum content width */
```

---

## 11. Component Library Structure: Atoms → Molecules → Organisms
...
│   │   └── tooltip.tsx
│   │
│   ├── commerce/                  # MOLECULES + ORGANISMS (e-commerce)
│   │   ├── ProductCard.tsx        # molecule
│   │   ├── RoastBadge.tsx         # atom (domain-specific)
│   │   ├── PriceDisplay.tsx       # atom (domain-specific)
│   │   ├── QuantitySelector.tsx   # molecule
│   │   ├── GrindSelector.tsx      # molecule
│   │   ├── AddToCartButton.tsx    # molecule
│   │   ├── CartDrawer.tsx         # organism
│   │   ├── CartItem.tsx           # molecule
│   │   ├── ProductGrid.tsx        # organism
│   │   ├── FilterSidebar.tsx      # organism
│   │   ├── SubscriptionToggle.tsx # molecule
│   │   └── FlavorProfileChart.tsx # molecule
│   │
│   ├── crm/                      # MOLECULES + ORGANISMS (CRM)
│   │   ├── CustomerCard.tsx       # molecule
│   │   ├── LeadStatusBadge.tsx    # atom (domain-specific)
│   │   ├── ActivityTimeline.tsx   # organism
│   │   ├── MetricCard.tsx         # molecule
│   │   ├── DataTable.tsx          # organism
│   │   ├── FilterBar.tsx          # organism
│   │   ├── NotesPanel.tsx         # organism
│   │   └── OrderHistoryList.tsx   # organism
│   │
│   └── layout/                    # TEMPLATES
...
│   └── tokens.ts                  # Exported token values for JS
│
├── styles/
...
| Rule | Example |
|------|---------|
| Atoms (shadcn/ui): lowercase kebab files | `button.tsx`, `input.tsx` |
| Domain components: PascalCase files | `ProductCard.tsx`, `CustomerCard.tsx` |
| Stories: adjacent, same name | `ProductCard.stories.tsx` |
| Tests: adjacent, same name | `ProductCard.test.tsx` |
| Variants: defined inside component via CVA | `variant: "default" | "compact"` |

### Component API Principles

1. **Composable over monolithic**: Prefer `<Card><CardHeader /><CardContent /></Card>` over `<Card title="..." content="..." />`
2. **Variants over props**: Use `variant="compact"` instead of `isCompact={true}`
3. **Slot patterns**: Allow injection of custom content via children and render props
4. **Forward refs**: All interactive components forward refs for form libraries and focus management
5. **Accessible defaults**: Every interactive component includes ARIA attributes, keyboard handling, and focus visible styles

---

## Summary & Recommendations for YourWave v2

### Key Decisions to Make

1. **Token format**: Use CSS custom properties with HSL values (the shadcn/ui convention). This gives you dark mode for free and works natively with Tailwind.

2. **Component ownership**: Copy-paste shadcn/ui components, then extend. Do not wrap them — own them. Create domain-specific components (ProductCard, CustomerCard) as compositions of base components.

3. **Storybook from day one**: Every component gets a story before it gets used in a page. Stories serve as documentation, visual tests, and the component API contract.

4. **Two domains, one system**: E-commerce and CRM share the same token layer and base components. Domain-specific components live in separate directories but use the same design language.

5. **Progressive dark mode**: Implement the token architecture for dark mode now (CSS variables, semantic naming), even if dark mode ships later. Retrofitting is 10x harder than building it in.

### Implementation Priority

```
Phase 1: Foundation
├── Tailwind config with YourWave tokens
├── CSS variables in globals.css (light + dark)
├── Install core shadcn/ui components (button, input, card, table, dialog)
├── Set up Storybook with theme switching
└── Typography and spacing utilities

Phase 2: Commerce Components
├── ProductCard, PriceDisplay, RoastBadge
├── CartDrawer, CartItem, QuantitySelector
├── ProductGrid, FilterSidebar
└── SubscriptionToggle

Phase 3: CRM Components
├── CustomerCard, LeadStatusBadge
├── DataTable, FilterBar
├── MetricCard, ActivityTimeline
└── OrderHistoryList, NotesPanel

Phase 4: Patterns & Templates
├── StorefrontLayout, DashboardLayout
...
*Research compiled for YourWave v2 Design System initiative. This document should be treated as a living reference — update as implementation decisions are made and patterns evolve.*
